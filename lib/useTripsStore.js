...
export function useTripsStore(userId, userEmail) {
  ...
  useEffect(() => {
    ...
    function checkLoaded() {
      if (ownDone && sharedDone) {
        setTrips([...ownTripsData, ...sharedTripsData]);
        setLoaded(true);
      }
    }
    ...
    async function seedOwn() {
      try {
        ...
        if (!snap.exists()) await setDoc(ownRef, { trips: [INITIAL_TRIP] });
      } catch (err) {
        console.error("Error inicializando viajes:", err);
      }
    }
    seedOwn();
    ...
    const unsubOwn = onSnapshot(
      ownRef,
      (snap) => {
        ownTripsData = snap.exists() ? (snap.data().trips || []).map((t) => ({ ...t, _isShared: false })) : [];
        if (!ownTripsData.some((t) => t.id === INITIAL_TRIP.id)) {
          ownTripsData = [INITIAL_TRIP, ...ownTripsData];
        }
        ownDone = true;
        checkLoaded();
      },
