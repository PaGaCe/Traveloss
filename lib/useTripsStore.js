...
export function useTripsStore(userId, userEmail) {
  ...
  useEffect(() => {
    ...
    const unsubOwn = onSnapshot(
      ownRef,
      async (snap) => {
        let trips = snap.exists() ? (snap.data().trips || []).map((t) => ({ ...t, _isShared: false })) : [];
        if (!trips.some((t) => t.id === INITIAL_TRIP.id)) {
          trips = [INITIAL_TRIP, ...trips];
          await setDoc(ownRef, { trips: trips.map(({ _isShared, ...rest }) => rest) }).catch(() => {});
        }
        ownTripsData = trips;
        ownDone = true;
        checkLoaded();
