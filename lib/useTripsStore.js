...
// ---------- Ordenación de días por fecha ----------
// Las fechas se guardan en formato "d mes" (p. ej. "3 sep", "10 Sep"). Los días
// sin fecha quedan al final para no romper el orden.
const MONTH_INDEX = {
...
function daySortValue(day) {
  const raw = String(day?.date || "").trim().toLowerCase();
  const parts = raw.split(/[\s\/\-]+/);
  if (parts.length < 2) return Infinity;
  const d = parseInt(parts[0], 10);
  const m = MONTH_INDEX[parts[1]];
  if (isNaN(d) || m === undefined) return Infinity;
  return m * 100 + d;
}

function sortDays(days) {
  if (!Array.isArray(days)) return [];
  return [...days].sort((a, b) => daySortValue(a) - daySortValue(b));
}
...
export function useTripsStore(userId, userEmail) {
  ...
  useEffect(() => {
    ...
    // Los listeners se activan ANTES que cualquier await: si la red va lenta,
    // los datos aparecen en cuanto llega el primer snapshot y la app nunca se
    // queda colgada en "Cargando..." esperando la migración o los descartados.
    // Antes init() era secuencial (descartados → migración → listeners) y con
    // red lenta el usuario tenía que cerrar y reabrir la app para ver sus viajes.
    ...
    // 0. Cargar viajes descartados del usuario en segundo plano (no bloquea)
    (async () => {
      try {
        const userSnap = await getDoc(doc(db, "users", userId));
        if (!cancelled && userSnap.exists() && userSnap.data().dismissedTrips) {
          const dismissed = userSnap.data().dismissedTrips;
          dismissedRef.current = dismissed;
          setDismissedTrips(dismissed);
          // Si algún viaje descartado llegó a mostrarse antes de cargar la
          // lista, ocultarlo ahora
          setTrips((prev) => prev.filter((t) => !dismissed.includes(t.id)));
        }
      } catch (err) {
    ...
    // 1. Migrar datos del antiguo formato (documento único) al nuevo
    //    (subcolección), en segundo plano y sin bloquear la carga.
    //    Solo migra viajes que no existan ya en la subcolección.
    (async () => {
      try {
  ...
  function mutateTrip(tripId, updater) {
    ...
    if (!trip) return null;
    let updated = updater(trip);
    // Siempre mantener los días ordenados por fecha tras cualquier cambio.
    if (updated) updated = { ...updated, days: sortDays(updated.days) };
  ...
  const getTrip = useCallback((tripId) => {
    const t = tripsRef.current.find((x) => x.id === tripId) || null;
    // Ordenar también a la lectura: datos viejos o de otro dispositivo con los
    // días desordenados se muestran por fecha aunque aún no se haya editado nada.
    return t ? { ...t, days: sortDays(t.days || []) } : null;
  }, []);
