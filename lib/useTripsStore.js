...
export function useTripsStore(userId, userEmail) {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      // 1. Activar listeners en tiempo real
  ...
  const persistOwn = useCallback(
    (nextOwnTrips) => {
      if (firebaseReady && userId) {
        setDoc(doc(db, "userTrips", userId), { trips: nextOwnTrips }).catch((err) => {
        console.error("Error guardando viajes propios:", err);
      });
      }
    },
    [userId]
  ...
  const unshareTrip = useCallback(
    async (tripId, targetUid) => {
      ...
      if (newSharedWith.length <= 1) {
        ...
        setTrips(next);
        await setDoc(doc(db, "userTrips", userId), { trips: [...ownTrips, tripData] });
      } else {
