...
export function useTripsStore(userId, userEmail) {
    ...
    async function init() {
      ...
      try {
        ...
        if (userSnap.exists() && userSnap.data().dismissedTrips) {
          ...
          console.log("[DIAG] dismissedTrips cargadas:", dismissed);
      ...
      const unsubOwnNew = onSnapshot(
        ...
        (ss) => {
          if (cancelled) return;
          console.log("[DIAG] sub listener fired. userId:", userId, "docs:", ss.docs.length);
          ss.docs.forEach((d) => console.log("[DIAG] sub doc:", d.id, d.data().title));
      ...
      const unsubShared = onSnapshot(
        ...
        (ss) => {
          if (cancelled) return;
          console.log("[DIAG] shared listener fired. userId:", userId, "docs:", ss.docs.length, "dismissed:", dismissedRef.current);
          ss.docs.forEach((d) => {
            const data = d.data();
            console.log("[DIAG] shared doc:", d.id, "sharedWith:", data.sharedWith, "ownerId:", data.ownerId);
          });
  ...
  // Re-añadir un viaje que estaba descartado (p. ej. al volver a abrir la invitación)
