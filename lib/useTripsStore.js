...
export function useTripsStore(userId, userEmail) {
      ...
      const unsubOwnOld = onSnapshot(
        ...
        (s) => {
          ...
          setTrips((prev) => {
            ...
            const fromSub = currentOwn.filter((t) => t._source !== "legacy" || !newIds.has(t.id));
            return [...fromSub, ...legacyTrips, ...sharedOnly];
          });
        },
        (err) => {
      ...
      const unsubOwnNew = onSnapshot(
        ...
        (ss) => {
          ...
          setTrips((prev) => {
            ...
            const fromLegacy = currentOwn.filter((t) => t._source !== "sub" || !subIds.has(t.id));
            return [...fromLegacy, ...subTrips, ...sharedOnly];
          });
        },
        (err) => {
  ...
  function mutateTrip(tripId, updater) {
    ...
    if (trip._isShared) {
      ...
      // También actualizar el doc legacy para mantener compatibilidad
      const ownTrips = current.filter((t) => !t._isShared).map((t) =>
        t.id === tripId ? updated : t
      );
      setDoc(doc(db, "userTrips", userId), { trips: ownTrips }).catch((err) => {
        // Ignorar error de tamaño (el subcollection tiene los datos actualizados)
        if (err.code !== "FAILED_PRECONDITION") {
          console.error("Error actualizando doc legacy:", err);
        }
      });
    }
    return updated;
