...
import { Plus, MapPin, Calendar, Compass, Trash2, LogOut, Users } from "lucide-react";
  ...
  return (
    <div className="min-h-screen flex items-center justify-center py-6 px-2 bg-[#C5CAD6]">
      <div className="w-full max-w-[420px] bg-white rounded-[32px] shadow-2xl overflow-hidden min-h-[85vh] flex flex-col items-center justify-center px-8">
        ...
        <p className="text-slate text-[14px] mb-8 text-center">Tus viajes en un solo lugar</p>
        ...
        {error && <p className="text-coral text-[12.5px] mt-3 text-center">{error}</p>}
...
export default function HomePage() {
  ...
  const { user, userId, loading: authLoading, signInWithGoogle, signOut, authReady, error: authError } = useAuth();
  const { trips, loaded, addTrip, deleteTrip, usingFirebase } = useTripsStore(userId, user?.email);
  ...
  if (authReady && !user) {
    return <LoginScreen signInWithGoogle={signInWithGoogle} error={authError} />;
  }
        ...
        <div className="px-5 pt-6 pb-5 bg-gradient-to-br from-ink to-inkLight">
          <div className="flex items-center gap-2 mb-1">
            ...
            <div className="ml-auto flex items-center gap-2">
              {user && user.photoURL && (
                <img src={user.photoURL} alt="" className="w-7 h-7 rounded-full ring-2 ring-white/30" />
              )}
              {user && (
                <button onClick={signOut} className="text-white/50 hover:text-white transition-colors" title="Cerrar sesión">
                  <LogOut size={16} />
                </button>
              )}
            </div>
          </div>
          <p className="text-white/70 text-[12.5px] mt-0.5">Tus viajes en un solo lugar</p>
        ...
        <div className="flex-1 overflow-y-auto px-5 pt-4 pb-24 bg-cloud">
          {trips.length === 0 ? (
            <div className="text-center mt-16">
              ...
              <p className="text-[14px] text-ink font-medium">No hay viajes aún</p>
              <p className="text-[12.5px] text-slate mt-1">Crea tu primer viaje para empezar</p>
            </div>
          ) : (
            <div className="flex flex-col gap-3">
              {trips.map((trip) => (
                ...
                >
                  {trip.image ? (
                    <>
                      <img src={trip.image} alt="" className="w-full h-36 object-cover" />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent" />
                      <div className="absolute inset-0 flex items-end px-4 py-3">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5">
                            <p className="font-semibold text-[16px] text-white font-display">{trip.title}</p>
                            {trip._isShared && <Users size={12} className="text-white/70" />}
                          </div>
                          ...
                          <p className="text-[11.5px] text-white/65 flex items-center gap-1 mt-0.5">
                            <Calendar size={10} /> {trip.dateLabel} · {trip.days.length} día{trip.days.length !== 1 ? "s" : ""}
                          </p>
                        </div>
                        {!trip._isShared && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              if (confirm(`¿Eliminar "${trip.title}"?`)) deleteTrip(trip.id);
                            }}
                    ...
                    <div className="flex bg-white">
                      <div className="w-2 shrink-0" style={{ background: trip.stampColor }} />
                      <div className="flex-1 px-4 py-3.5">
                        <div className="flex items-start justify-between">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1.5">
                              <p className="font-semibold text-[16px] text-ink font-display">{trip.title}</p>
                            ...
                            <p className="text-[12px] flex items-center gap-1 mt-0.5 text-slate">
                              <Calendar size={11} /> {trip.dateLabel} · {trip.days.length} día{trip.days.length !== 1 ? "s" : ""}
                            </p>
                          </div>
                          {!trip._isShared && (
                            <button
